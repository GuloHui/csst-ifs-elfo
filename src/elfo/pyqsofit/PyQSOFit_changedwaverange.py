"""
Identifier:     pyqsofit
Name:           PyQSOfit_changedwaverange.py
Description:
Author:
Created:
Modified-History:
...
class QSOFit():
    ...
    def Fit(self, name=None, nsmooth=1, and_mask=False, or_mask=False, reject_badpix=True, deredden=True, wave_range=None,
        ...
        # Clean and/or mask
        if (and_mask is True) and (self.and_mask_in is not None):
            self._MaskSdssAndOr(self.lam, self.flux, self.err, and_mask, or_mask)
        # Clean bad pixel
        if reject_badpix is True:
        ...
        # Deredden
        if deredden is True and self.ra != -999. and self.dec != -999.:
    ...
    def _MaskSdssAndOr(self, lam, flux, err, and_mask, or_mask):
        ...
        """
        if (and_mask is True) and (or_mask is True):
            ind = np.where((self.and_mask_in == 0) & (self.and_mask_in == 0), True, False)
        if (and_mask is True) and (or_mask is False):
            ind = np.where(self.and_mask_in == 0, True, False)
        if (and_mask is False) and (or_mask is True):
    ...
    def _CalculateSN(self, wave, flux, alter=True):
        ...
        if np.all(np.array([np.sum(ind5100), np.sum(ind3000), np.sum(ind1350)]) < 10):
            ...
            if not np.all(np.isnan(tmp_SN)):
                ...
                self.SN_ratio_conti = -1.
            # print('flux[ind5100].std(),flux[ind3000].std(),flux[ind1350].std()，flux[ind5100].mean(),np.nanmean(tmp_SN):',flux[ind5100].std(),flux[ind3000].std(),flux[ind1350].std(),flux[ind5100].mean(),np.nanmean(tmp_SN))
    ...
    def fit_continuum(self, wave, flux, err, ra, dec, plateid, mjd, fiberid):
        ...
        if (self.Fe_uv_op is False) or (np.sum(ind_uv) <= self.n_pix_min_conti):
        ...
        if (self.Fe_uv_op is False and self.BC is False) or (np.sum(ind_opt) <= self.n_pix_min_conti):
        ...
        if (self.BC is False) or (np.sum(ind_BC) <= 100):
        ...
        if self.Fe_uv_op is True and self.poly is False and self.BC is False:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Fe_flux_mgii(xval, pp[0:3]) + self.Fe_flux_balmer(xval, pp[3:6])
        elif self.Fe_uv_op is True and self.poly is True and self.BC is False:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Fe_flux_mgii(xval, pp[0:3]) + self.Fe_flux_balmer(xval, pp[3:6]) + self.F_poly_conti(xval, pp[11:])
        elif self.Fe_uv_op is True and self.poly is False and self.BC is True:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Fe_flux_mgii(xval, pp[0:3]) + self.Fe_flux_balmer(xval, pp[3:6]) + self.Balmer_conti(xval, pp[8:11])
        elif self.Fe_uv_op is False and self.poly is True and self.BC is False:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.F_poly_conti(xval, pp[11:])
        elif self.Fe_uv_op is False and self.poly is False and self.BC is False:
            def _conti_model(xval, pp):
                return self.PL(xval, pp)
        elif self.Fe_uv_op is False and self.poly is False and self.BC is True:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Balmer_conti(xval, pp[8:11])
        elif self.Fe_uv_op is True and self.poly is True and self.BC is True:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Fe_flux_mgii(xval, pp[0:3]) + self.Fe_flux_balmer(xval, pp[3:6]) + self.F_poly_conti(xval, pp[11:]) + self.Balmer_conti(xval, pp[8:11])
        elif self.Fe_uv_op is False and self.poly is True and self.BC is True:
            def _conti_model(xval, pp):
                return self.PL(xval, pp) + self.Fe_flux_balmer(xval, pp[3:6]) + self.F_poly_conti(xval, pp[11:]) + self.Balmer_conti(xval, pp[8:11])
        else:
        ...
        """
        if (self.MCMC is True or self.MC is True) and self.nsamp > 0:

            if (self.MCMC is True) and (self.MC is False):
                """
                MCMC sampling
                """
            ...
            elif (self.MCMC is False) and (self.MC is True):
                """
                MC resampling
                """
    ...
    def fit_lines(self, wave, line_flux, err, f):
        ...
        if ind_kind_line.any():
            ...
            for ii in range(ncomp):
                ...
                ind_n = np.where((wave > comp_range[0]) & (wave < comp_range[1]) & (ind_neg_line), True, False)
                ...
                if np.sum(ind_n) > 10:
                    ...
                    for n in range(nline_fit):
                        ...
                        if ngauss_fit[n] > 0:
                            ...
                            for nn in range(ngauss_fit[n]):
                                ...
                                if abs(sig_0 - sig_low) < tolerance:
                                    sig_0 += tolerance
                                elif abs(sig_0 - sig_up) < tolerance:
                                    sig_0 -= tolerance

                                minvoff = -voff
                                maxvoff = voff
                                # print(f'minvoff:{minvoff}')
                                # print(f'maxvoff:{maxvoff}')

                                lowlimt = np.log(comp_range[0])
                                # print(f'lowlimt:{lowlimt}')
                                uplimit = np.log(comp_range[1])
                                # print(f'uplimt:{uplimit}')
                                # print(f'ln_lambda_0:{ln_lambda_0}')
                                lower_bound = ln_lambda_0 - voff
                                upper_bound = ln_lambda_0 + voff
                                # print(f'lower_bound:{lower_bound}')
                                # print(f'upper_bound:{upper_bound}')
                                # print(f'lowlimt - ln_lambda_0:{lowlimt - ln_lambda_0}')
                                # print(f'uplimit - ln_lambda_0:{uplimit - ln_lambda_0}')
                                # 检查是否超出范围

                                if lower_bound <= lowlimt+sig_low:
                                    minvoff = lowlimt - ln_lambda_0 + sig_low   # 调整最小速度偏移
                                    print(f"minvoff 调整-为: {minvoff}")
                                if upper_bound >= uplimit-sig_low:
                                    maxvoff = uplimit - ln_lambda_0 - sig_low  # 调整最大速度偏移
                                    print(f"maxvoff 调整为: {maxvoff}")

                                fit_params.add(f'{line_name}_{nn+1}_scale', value=scale_0, min=scale_low, max=scale_up, vary=vary)
                                fit_params.add(f'{line_name}_{nn+1}_dwave', value=dwave_0, min=minvoff, max=maxvoff, vary=vary)
                    ...
                    if (self.MCMC is True or self.MC is True) and self.nsamp > 0:
                        ...
                        if (self.MCMC is True) and (self.MC is False):
                            ...
                            flat_samples = line_samples.chain
                            ...
                            if self.plot_corner:
                                import corner
                                truths = [params_dict[k] for k in df_samples.columns.values.tolist()]
                                corner.corner(df_samples, labels=line_samples.var_names,
                                              quantiles=[0.16, 0.5, 0.84], truths=truths)
                        ...
                        MC resampling
                        """
                        if (self.MCMC is False) and (self.MC is True):
                        ...
                        if (self.MCMC is True) and (self.MC is True):
    ...
    def plot_fig(self, save_fig_path='.', broad_fwhm=1200, plot_line_name=True, plot_legend=True, ylims=None, plot_residual=True, show_title=True, plot_br_prop=False):
        ...
        # Plot lines
        if (self.linefit is True) & (len(self.line_result) > 0):
            ...
            gs = fig.add_gridspec(2, np.max([ncomp_fit, 1]))

            # 在第一行绘制一个大的子图
            ax = fig.add_subplot(gs[0, :])

            # 在第二行绘制多个小的子图
            axn = [fig.add_subplot(gs[1, i]) for i in range(ncomp_fit)]
            ...
            for c in range(ncomp_fit):
                ...
                axn[c].set_xticks([self.all_comp_range[2*c], np.round((self.all_comp_range[2*c] + self.all_comp_range[2*c+1])/2, -1), self.all_comp_range[2*c+1]])

                axn[c].text(0.02, 0.9, _pretty_name(self.uniq_linecomp_sort[c]), fontsize=20, transform=axn[c].transAxes)
                ...
                if plot_br_prop is True:
                    ...
                    if mc_flag == 2:
                        ...
                        axn[c].text(0.02, 0.75, fr'$L_{{\rm{{br}}}}=10^{{{{{np.round(np.log10(self.flux2L(area)), 2)}}}\pm{{{np.round(0.434*self.flux2L(area_err)/self.flux2L(area), 2)}}}}}$'+r'$\ \rm{erg}\ \rm{s}^{-1}$',
                                    fontsize=12, transform=axn[c].transAxes)
                        axn[c].text(0.02, 0.675, fr'${{\rm{{FWHM}}}}_{{\rm{{br}}}}={{{int(np.round(fwhm, 0))}}}\pm{{{int(np.round(fwhm_err, 0))}}}$'+r'$\ \rm{km}\ \rm{s}^{-1}$',
                                    fontsize=12, transform=axn[c].transAxes)

                    else:
                        axn[c].text(0.02, 0.75, fr'$L_{{\rm{{br}}}}=10^{{{np.round(np.log10(self.flux2L(area)), 1)}}}$'+r'$\ \rm{erg}\ \rm{s}^{-1}$',
                                    fontsize=12, transform=axn[c].transAxes)
                        axn[c].text(0.02, 0.675, fr'${{\rm{{FWHM}}}}_{{\rm{{br}}}}={{{int(np.round(fwhm, 0))}}}$'+r'$\ \rm{km}\ \rm{s}^{-1}$',
                                    fontsize=12, transform=axn[c].transAxes)
                ...
                if self.wave_mask is not None:
                    ...
                    for j, w in enumerate(self.wave_mask):
                        ...
                        if j == 0:
                            ...
                            label_data = 'data'
                            label_resid = 'resid'
                            axn[c].plot(self.wave[mask], self.line_flux[mask], 'k', label=label_data, lw=1, zorder=2)
                        ...
                        if plot_residual:
                            ...
                            axn[c].plot(self.wave[mask], self.line_flux[mask] - self.f_line_model[mask] - 5, 'gray',
        ...
        # Host decomposition
        if self.decompose_host is True and self.decomposed is True:
        ...
        if self.linefit is True:
            ...
            plot_bottom = np.min([-0.2, -3*mad])  # TODO: stupid absolute lower limit here
        else:
            if self.decomposed is False:
        ...
        if self.linefit is True:
            ...
            fig.supylabel(r'$\rm f_{\lambda}$ ($\rm 10^{-20} erg\;s^{-1}\;cm^{-2}\;\AA^{-1}$)', fontsize=20)
        else:
            fig.supxlabel(r'$\rm Rest \, Wavelength$ ($\rm \AA$)', fontsize=20)
            fig.supylabel(r'$\rm f_{\lambda}$ ($\rm 10^{-20} erg\;s^{-1}\;cm^{-2}\;\AA^{-1}$)', fontsize=20)
        # Save figure
